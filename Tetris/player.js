	...
	this.update = function() {
		if (!tetrisGame.gameOver) {
			...
			if (!tetrisGame.backgroundGame) {
				if (map.Button4 && !this.rotated) {
					this.rotate();
					this.rotated = true;
				}
				if (map.Button1 && this.canMoveRight && !this.rightPressed) {
				...
				if (map.Button2 && this.canMoveLeft && !this.leftPressed) {
					this.x -= blockWidth; //change base position (relative offset)
					for (let i = 0; i < this.blocks.length; i++) { //change block positions
						this.blocks[i].x -= blockWidth;
					}
				...
				if (this.rightPressed || this.leftPressed) {
					this.moveTimer++;
					if (this.moveTimer >= 20 && this.moveTimer % 5 == 0) {
						for (let i = 0; i < this.blocks.length; i++) { //change block positions
							if (this.rightPressed && this.canMoveRight) {
								this.blocks[i].x += blockWidth;
							}
