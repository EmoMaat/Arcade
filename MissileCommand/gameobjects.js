...
class plane {
	...
	update() {
		if (this.dir) {
			this.x += 1.1 * missileCommandGame.speedMultiplier * 0.75;
			ctx.save();
		...
		ctx.drawImage(this.sprite,0,0,this.sprite.width*0.75,this.sprite.height*0.75);
		ctx.restore();
...
class satelite {
	...
	update() {
		if (this.dir) {
			this.x += 1.5 * missileCommandGame.speedMultiplier * 0.75;
			ctx.save();
			ctx.translate(this.x,this.y);
			ctx.scale(1,1);
		...
		ctx.drawImage(this.sprite,0,0,this.sprite.width*0.8*0.75,this.sprite.height*0.8*0.75);
		ctx.restore();
