...
function MissileCommand() {
	...
	arcade.menu.object = arcade.game.object = new MissileGame(true);
	arcade.menu.object.buttons = [
		["EXIT", "loadHub()"],
		["HOW TO PLAY", "loadInstructions()"],
		["HIGH SCORES", "loadHighscores('" + currentGame + "', 0,true)"],
		["START GAME", "newMissileGame()"]
	];
}
...
function newMissileGame() {
	...
	arcade.game.object = new MissileGame();

	arcade.game.interval = new Interval(arcade.game.object.update,10);
}
...
class MissileGame {
	constructor(backgroundGame) {
		...
		this.missileBatteries = [new missileBattery(137.5, window.height - 75), new missileBattery(911.5, window.height - 75), new missileBattery(1789.5, window.height - 75)];
		this.smartBombs = [];
		this.planes = [];
		this.satelites = [];
		this.cities = [];
		...
		let Missilecanvas = document.createElement('canvas');
		Missilecanvas.id = 'MissileCanvas';
		Missilecanvas.width = window.width;
		Missilecanvas.height = window.height;			// is set later
		Missilecanvas.style.left = 0;
		Missilecanvas.style.bottom = 0;
		Missilecanvas.style.position = "absolute";
		Missilecanvas.style.cursor = "none";
		Missilecanvas.style.zIndex = 1;
		document.body.appendChild(Missilecanvas);

		this.canvas = document.getElementById("MissileCanvas");		// window stuff
	...
	update() {
		...
		if (!self.backgroundGame) {
			...
			this.ctx.translate(0,screen.height*0.35);
			self.drawCities();
			self.drawGround();
	...
	roundStartOverlay() {
		...
		if (!self.pushedCities) {
			...
			if (self.cityCheck[0]) {
				self.cities.push(new city(290,window.height - 95,false));
			} else {
				self.cities.push(new city(290,window.height - 95,true));
			}
			if (self.cityCheck[1]) {
				self.cities.push(new city(487,window.height - 85,false));
			} else {
				self.cities.push(new city(487,window.height - 85,true));
			}
			if (self.cityCheck[2]) {
				self.cities.push(new city(660,window.height - 80,false));
			} else {
				self.cities.push(new city(660,window.height - 80,true));
			}
			if (self.cityCheck[3]) {
				self.cities.push(new city(1045,window.height - 95,false));
			} else {
				self.cities.push(new city(1045,window.height - 95,true));
			}
			if (self.cityCheck[4]) {
				self.cities.push(new city(1290,window.height - 120,false));
			} else {
				self.cities.push(new city(1290,window.height - 120,true));
			}
			if (self.cityCheck[5]) {
				self.cities.push(new city(1500,window.height - 90,false));
			} else {
				self.cities.push(new city(1500,window.height - 90,true));
			}
			self.pushedCities = true;
			self.enemyTargets = self.cities.concat(self.missileBatteries);
		}

		self.missileBatteries = [new missileBattery(137.5, window.height - 75), new missileBattery(911.5, window.height - 75), new missileBattery(1789.5, window.height - 75)];
		if (!self.backgroundGame) {
			this.roundStartTimer++;
			if (this.roundStartTimer > 40) {
				this.ctx.font = "50px segoe ui";
				this.ctx.textAlign = "center";
				this.ctx.fillText("ROUND " + self.wave,window.width/2,window.height / 4);

			}
			if (this.roundStartTimer > 90) {
				this.ctx.fillText(self.waveMultiplier + "x  POINTS",window.width/2,window.height/4 + 130);
			}
			if (this.roundStartTimer > 150) {
				this.ctx.textAlign = "center";
				this.ctx.fillText("DEFEND",window.width/3,window.height/5*3);
				this.ctx.fillText("CITIES",window.width/3*2,window.height/5*3);
			}
	...
	updateGameObjects() {
		...
		for (let i = 0; i < self.planes.length; i++) {
			...
			if (self.planes[i].dir && self.planes[i].x > window.width + 90) {
		...
		for (let i = 0; i < self.satelites.length; i++) {
			...
			if (self.satelites[i].dir && self.satelites[i].x > window.width + 90) {
	...
	finishRound() {
		...
		if (!self.finished) {
			...
			if (self.roundEndTimer > 20) {
				...
				this.ctx.textAlign = "center";
				this.ctx.fillText("BONUS POINTS",window.width/2,window.height/4);
			}
			if (self.roundEndTimer > 50) {
				this.ctx.font = "35px segoe ui";
				this.ctx.textAlign = "left";
				this.ctx.fillText(self.missilePoints,window.width/2 - 320,window.height/4 + 105);
				//draw all bonus missiles
				for (let i = 0; i < self.bonusMissiles; i++) {
					this.ctx.fillStyle = "#fff";
					this.ctx.drawImage(self.bonusMissileImg,window.width/2 - 250 + i*20, window.height/4 + 73,self.bonusMissileImg.width*0.8,self.bonusMissileImg.height*0.8);
				}
			}
			...
			if (self.roundEndTimer > 100) {
				this.ctx.fillText(self.cityPoints,window.width/2 - 320,window.height/4 + 300);
			...
			if (self.awardCities) {
				if (self.roundEndTimer > 150) {
					this.ctx.textAlign = "center";
					this.ctx.font = "50px segoe ui";
					this.ctx.fillText("BONUS CITY!",window.width/2,window.height/4 + 480);
				}
				if (self.roundEndTimer == 150) {
	...
	startCityCount() {
		...
		if (self.counting) {
			if (self.countTimer < 23) {
				...
				if (self.cityCheck[self.cityIndex]) {
					self.bonusCities.push(new city(window.width/2 -200 + (self.bonusCities.length * 150),window.height/2 + 20));
	...
	drawGround() {
		this.ctx.scale(1,1);
		this.ctx.beginPath();
		this.ctx.moveTo(0,window.height);
		this.ctx.lineTo(0,window.height - 100);
		this.ctx.lineTo(70,window.height - 125);
		this.ctx.bezierCurveTo(72,window.height -145,100,window.height -150,110,window.height -133);
		this.ctx.lineTo(205,window.height - 133); //center line of left missile battery
		this.ctx.quadraticCurveTo(225,window.height - 165,245,window.height - 133);
		this.ctx.lineTo(290,window.height - 85); //1st city pos
		this.ctx.lineTo(400,window.height - 85);
		this.ctx.quadraticCurveTo(420,window.height - 120,450,window.height - 85); //2nd city
		this.ctx.lineTo(660,window.height - 85); //660 > 780
		this.ctx.bezierCurveTo(680,window.height - 60,760,window.height - 60,780,window.height - 85); //3rd city from l's crater
		this.ctx.lineTo(820,window.height - 95);
		this.ctx.lineTo(870,window.height - 130);
		this.ctx.quadraticCurveTo(880,window.height-155,905,window.height - 130); //middle left boble
		this.ctx.lineTo(953,window.height - 130); //center line of middle missile battery
		this.ctx.quadraticCurveTo(968,window.height - 158,995,window.height - 130) //middle right bobble
		this.ctx.lineTo(1017,window.height - 95);
		this.ctx.lineTo(1045,window.height - 95); //1045 > 1170
		this.ctx.bezierCurveTo(1065,window.height - 70,1150,window.height - 70,1170,window.height - 95); //4th city's crater
		this.ctx.lineTo(1260,window.height - 95);
		this.ctx.lineTo(1290,window.height - 120);
		this.ctx.quadraticCurveTo(1345,window.height - 95,1400,window.height - 120); //5th city's crater
		this.ctx.lineTo(1417,window.height - 108);
		this.ctx.lineTo(1427,window.height - 90);
		this.ctx.lineTo(1500,window.height - 90);
		this.ctx.bezierCurveTo(1520,window.height - 65,1605,window.height - 65,1625,window.height - 90); //6th city's crater
		this.ctx.lineTo(1675,window.height - 95);
		this.ctx.lineTo(1718,window.height - 125);
		this.ctx.bezierCurveTo(1718,window.height - 142,1755,window.height - 150,1764,window.height - 135);
		this.ctx.lineTo(1860,window.height - 135);
		this.ctx.bezierCurveTo(1860,window.height-142,1875,window.height - 155,1895,window.height - 132);
		this.ctx.lineTo(window.width + 500,window.height - 50);
		this.ctx.lineTo(window.width + 500,window.height);
		this.ctx.closePath();
		this.ctx.fillStyle = "#fff";
		this.ctx.fill();
	}
	...
	drawScore() {
		...
		this.ctx.fillStyle = "#fff";
		this.ctx.textAlign = "center";
		this.ctx.strokeText(self.score,window.width/4,60);
		this.ctx.fillText(self.score,window.width/4,60);
	}
	quit() {
		planeSound.pause();
		cityCountSound.pause();
...
class targetCursor {
	...
	handleMove() {
		...
		if (move.right && this.x + 2*this.size < window.width) {
			this.x += 8.5 * cursorSpeedMultiplier;
		}
		if (move.left && this.x - 2*this.size > 0) {
			this.x -= 8.5 * cursorSpeedMultiplier;
		}
		if (move.down && this.y < window.height - 200) {
