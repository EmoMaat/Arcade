...
let LunarLanderCanvas = document.createElement('canvas');
LunarLanderCanvas.id = 'LunarLanderCanvas';
LunarLanderCanvas.width = window.width;
LunarLanderCanvas.height = window.height;			// is set later
LunarLanderCanvas.style.left = 0;
LunarLanderCanvas.style.bottom = 0;
LunarLanderCanvas.style.position = "absolute";
LunarLanderCanvas.style.cursor = "none";
LunarLanderCanvas.style.zIndex = 1;
document.body.appendChild(LunarLanderCanvas);

let canvas = document.getElementById("LunarLanderCanvas");		// canvas stuff
let ctx = canvas.getContext("2d");	// canvas stuff

function LunarLander() {
	new MenuInterface();
	LunarLanderMainMenuOverlay();

	arcade.menu.object.backgroundGame = arcade.game.object = new lunarLanderGame(true);

	overlayInterval = new Interval(function(){
		ctx.clearRect(0, 0, canvas.width, canvas.height);
		if (lunarScreen == 2) {
			ctx.fillStyle = "#000";
			ctx.fillRect(700,canvas.height-370,1000,310);
			ctx.lineWidth = 2;
			ctx.strokeRect(700,canvas.height-370,1000,310);
			...
			switch (menuOverlay.selected) {
				case 0:
					ctx.fillText("TRAINING MISSION",1200,canvas.height - 400);
					...
					break;
				case 1:
					ctx.fillText("CADET MISSION",1200,canvas.height - 400);
					ctx.font = "30px segoe ui";
					ctx.textAlign = "left";
					...
					break;
				case 2:
					ctx.fillText("PRIME MISSION",1200,canvas.height - 400);
					ctx.font = "30px segoe ui";
					ctx.textAlign = "left";
					...
					break;
				case 3:
					ctx.fillText("COMMAND MISSION",1200,canvas.height - 400);
					ctx.font = "30px segoe ui";
					ctx.textAlign = "left";
					ctx.fillText("Moderate gravity",1030,canvas.height - 285);
					ctx.fillText("No friction",1030,canvas.height - 235);
...
function LunarLanderMainMenuOverlay() {
	...
	arcade.menu.object.buttons = [
		["EXIT", "loadHub()"],
		["HIGHSCORES", "loadHighscores('" + currentGame + "', 0)"],
		["SELECT MISSION", "loadMissionButtons()"]
	];
}
function loadMissionButtons() {
	...
	lunarScreen = 2
	arcade.menu.object.buttons = [
		["COMMAND MISSION", "selectMission(4)"],
		["PRIME MISSION", "selectMission(3)"],
		["CADET MISSION", "selectMission(2)"],
		["TRAINING MISSION", "selectMission(1)"]
	];
}
function selectMission(mission) {
	lunarDifficulty = mission;
	newarcade.game.object();
}
...
function newlunarLanderGame(background) {
	...
	arcade.game.object = new arcade.game.object(false);
...
class lunarLanderGame {
	...
	spawnLander() {
		for (let i = 0; i < arcade.game.object.map.fullmap.length - 1; i++) {
			if (arcade.game.object.map.fullmap[i+1].x > this.lander.x && arcade.game.object.map.fullmap[i-1].x < this.lander.x) {
		...
		this.map.offsetY = -arcade.game.object.lander.y + 100;
		this.map.offsetX = arcade.game.object.lander.x;
