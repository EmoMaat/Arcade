class terrainMap {
	...
	render() {
		...
		for (let i = 0; i < this.landingSpots2x.length; i++) {
			...
			if (this.landingSpots2x[i].y1 > canvas.height - 50) {
				let dx = arcade.game.object.lander.x - (0.5*(this.landingSpots2x[i].x1 + this.landingSpots2x[i].x2));
				let dy = arcade.game.object.lander.y - this.landingSpots2x[i].y1;
				let dist = Math.floor(Math.sqrt(dx*dx + dy*dy) / arcade.game.object.zoom);
		...
		for (let i = 0; i < this.landingSpots5x.length; i++) {
			...
			if (this.landingSpots5x[i].y1 > canvas.height - 50) {
				ctx.textAlign = "center";
				let dx = arcade.game.object.lander.x - (0.5*(this.landingSpots5x[i].x1 + this.landingSpots5x[i].x2));
				let dy = arcade.game.object.lander.y - this.landingSpots5x[i].y1;
				let dist = Math.floor(Math.sqrt(dx*dx + dy*dy) / arcade.game.object.zoom);
				ctx.font = "20px segoe ui";
		...
		for (let i = 0; i < this.landingSpots4x.length; i++) {
			...
			if (this.landingSpots4x[i].y1 > canvas.height - 50) {
				ctx.textAlign = "center";
				let dx = arcade.game.object.lander.x - (0.5*(this.landingSpots4x[i].x1 + this.landingSpots4x[i].x2));
				let dy = arcade.game.object.lander.y - this.landingSpots4x[i].y1;
				let dist = Math.floor(Math.sqrt(dx*dx + dy*dy) / arcade.game.object.zoom);
				ctx.font = "20px segoe ui";
	...
	fixOffset() {
		...
		arcade.game.object.lander.x += this.offsetX;
		arcade.game.object.lander.y += this.offsetY;
	}
	...
	zoomIn(px,py,zoomlvl) {
		...
		arcade.game.object.g *= zoom;

		this.offsetY = -arcade.game.object.lander.y*zoom + 200; //150 = zooming altitude/distance
		this.offsetX = -arcade.game.object.lander.x*zoom + canvas.width/2;

		arcade.game.object.lander.x *= zoom;
		arcade.game.object.lander.y *= zoom;

		arcade.game.object.lander.throttle *= zoom;
		arcade.game.object.lander.thrust *= zoom;
		arcade.game.object.lander.maxThrust *= zoom;

		//zooming out makes you go too fast, so only apply to zooming in
		if (zoom > 1) {
			arcade.game.object.lander.velX *= zoom;
			arcade.game.object.lander.velY *= zoom;
		}
	}
