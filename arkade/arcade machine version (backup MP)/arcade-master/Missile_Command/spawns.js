function spawnMissile(split,pos) {
	...
	this.fixedTarget = {x: this.target.x*0.75,y:this.target.y};
	if (this.toSpawn) {
		//now make the target's position the center of the object:
		if (this.target.targetType == "city") {
			this.tarPos = {
				x: 0.5*(this.fixedTarget.x + (this.fixedTarget.x + (125*0.75))),
				y: this.fixedTarget.y
			};
		} else {
			this.tarPos = {
				x: this.fixedTarget.x + 15*0.75,
				y: this.fixedTarget.y - 20*0.75
			};
		}
		if (this.target != "") {
			if (this.split) {
