	...
	handleFire() {
		if (this.player == 0) {
			if (this.canFire && (map[96] || map.Button4)) {
				asteroidsGame.projectiles.push(new projectile(this.px,this.py,this.radians - Math.PI,true));
				this.canFire = false;
			}
		} else {
...
class asteroid {
	...
	split() {
		if (this.splitLvl > 0) {
			asteroidsGame.asteroids.push(new asteroid(this.x,this.y,this.radians + 0.3,this.speed*1.1,this.splitLvl - 1));
			asteroidsGame.asteroids.push(new asteroid(this.x,this.y,this.radians - 0.3,this.speed*1.1,this.splitLvl - 1));
		}
		this.del = true;
	}
