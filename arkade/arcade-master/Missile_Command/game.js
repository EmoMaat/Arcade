...
function MissileCommandMainMenuOverlay() {
	move.smooth = false;
	menuOverlay.buttons = [
		new Button(120, canvas.height - 230, 400, 30, "START GAME", "newMissileGame()"),
...
class MissileGame {
	...
	handleCityAward() {
		console.log(self.availableBonusCities, self.cities.length);
	...
	roundStartOverlay() {
		...
		if (!self.pushedCities) {
			soundPlay("Missile_Command/src/sounds/roundstart.mp3",1);
			if (self.cityCheck[0]) {
	...
	updateGameObjects() {
		...
		for (let i = 0; i < self.explosions.length; i++) {
			...
			for (let c = 0; c < self.cities.length; c++) {
				let dx = self.cities[c].centerX - self.explosions[i].x;
			...
			for (let b = 0; b < self.missileBatteries.length; b++) {
				let dx = self.missileBatteries[b].centerX - self.explosions[i].x;
		...
		if ((self.planes.length > 0 || self.satelites.length > 0)) {
			planeSound.play();
			console.log(planeSound.currentTime);
		} else {
			planeSound.pause();
		}
	}
		...
		if (!self.finished) {
			...
			if (self.awardCities) {
				...
				if (self.roundEndTimer == 150) {
					...
					if (Math.random() > 0.5) {
						soundPlay("Missile_Command/src/sounds/cityaward1.mp3",1);
					} else {
						soundPlay("Missile_Command/src/sounds/cityaward2.mp3",1);
					}
				}
	...
	startMissileCount() {
		...
		if (self.counting) {
			if (self.countTimer < 9) {
				...
				soundPlay("Missile_Command/src/sounds/missilecount.mp3",1);
		...
		if (self.counting) {
			if (self.countTimer < 23) {
				...
				if (self.cityCheck[self.cityIndex]) {
					...
					self.score += 100*self.waveMultiplier;
					soundPlay("Missile_Command/src/sounds/citycount.mp3",1);
				}
				self.countTimer = 0;
			...
			if (!self.fired[1]) {
				if (map.Button0 || map[88]) { // fire from middle battery (using 'x' key)
					if (self.missileBatteries[1].missiles > 0) {
						self.missiles.push(new missile(929,screen.height-135,this.target,14*missileCommandGame.speedMultiplier,true));
						self.missileBatteries[1].missiles--;
					}
					if (self.missileBatteries[1].missiles <= 0) {
						soundPlay("Missile_Command/src/sounds/empty.mp3",1);
					} else {
						soundPlay("Missile_Command/src/sounds/fire.mp3",0.8);
					}
					self.fired[1] = true;
				}
			}
			if (!self.fired[0]) {
				if (map.Button2 || map[90]) { //fire from left battery (using 'z' key)
					if (self.missileBatteries[0].missiles > 0) {
						self.missiles.push(new missile(150,screen.height-133,this.target,6.5*missileCommandGame.speedMultiplier,true));
			...
			if (!self.fired[2]) {
				if (map.Button1 || map[67]) { //fire from right battery (using 'c' key)
					if (self.missileBatteries[2].missiles > 0) {
						self.missiles.push(new missile(1812,screen.height-135,this.target,6.5*missileCommandGame.speedMultiplier,true));
	...
	drawGround() {
		...
		ctx.lineTo(canvas.width,canvas.height - 105);
