	...
	update() {
		...
		for (var i = 0; i < this.buttons.length; i++) {
			if (i == this.selected) { // the selected one should look different
				...
				if (map.Button0 || map[13]) { // if we want to use one of the buttons
					map[13] = false;
					map.Button0 = false;
...
class EscOverlay {
	constructor() {
		...
		this.oldGameInterval = 15;		// save the update interval so it continues with the correct speed
	...
	update() {
		...
		if (this.blink <= 50) {
			this.overlayCtx.font = '48pt verdana';
...
class Interval {
	...
	pause() {
		// if (this.state != 0) return; // only execute if the code is running
		//line above seems to not make the function work^^
