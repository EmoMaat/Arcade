...
function SnakeMainMenuOverlay() {
	menuOverlay.buttons = [
		new Button(120, canvas.height - 230, 400, 30, "START GAME", "newSnakeGame()"),
...
class SnakeGame {
	...
	update() {
		ctx.clearRect(0,0,canvas.width,canvas.height);
		self.snake.update();
		if (self.snake.x == self.snakeFood.x - (snakeWidth / 2) - 32 && self.snake.y == self.snakeFood.y - (snakeHeight / 2) - 32) {
			self.snake.length++;
			self.newFood();
	...
	gameOverScreen() {
		...
		if (self.screenCounter > 160) {
			loadHighscores(self.snake.length, currentGame);
		}
	}
}
