...
function MidnightMotoristsMainMenuOverlay(){
	menuOverlay.buttons = [
		new Button(120, canvas.height - 230, 400, 30, "START GAME", "newMidnightMotoristsGame()"),
...
class MidnightMotoristsGame{
	constructor(){
        ...
        this.roadlanes = [110, 210, 360, 460, 620, 720, 870, 970]; // 0-7

        // contains the places where the laps should be drawn in this.distance. should be multidimensional to set flags
        this.laps = [[30000, 0], [60000, 0], [90000, 0], [120000, 0]];
        //this.laps = [[3000, 0], [6000, 0], [9000, 0], [12000, 0]];
        ...
        this.player = new MidnightMotoristsPlayer(300, this.roadlanes, this.rainTheme);
    ...
    carGenerationHandler(){
        ...
        var chance = this.cartimer * 0.0025 * (1 - this.cars.length / this.maxcars) * (1 + 0.65 * this.player.lap) * ((this.distance + this.player.x + this.player.width - oldlapdistance) / (this.laps[this.player.lap][0] - oldlapdistance)) * 7.5;
    ...
    draw_road(){
        // stationary lines
        // top
        this.drawLine(0, canvas.height / 36, canvas.width, canvas.height / 36, 50, 7);
        this.drawLine(0, canvas.height / 25, canvas.width, canvas.height / 25, 255, 14);
        this.drawLine(0, canvas.height / 19.5, canvas.width, canvas.height / 19.5, 50, 7);

        // middle
        this.drawLine(0, canvas.height / 2.049, canvas.width, canvas.height / 2.049, 50, 7);
        this.drawLine(0, canvas.height / 2, canvas.width, canvas.height / 2, 255, 14);
        this.drawLine(0, canvas.height / 1.953, canvas.width, canvas.height / 1.953, 50, 7);
        ...
        for(var i = 0; i < this.roadlines.length; i++){
            ...
            this.drawLine(this.roadlines[i][0] + 9, canvas.height / 4 + 14, this.roadlines[i][1] - 9, canvas.height / 4 + 14, 255, 14);

            // horizontal grey lines
            this.drawLine(this.roadlines[i][0], canvas.height / 4.2 + 14, this.roadlines[i][1], canvas.height / 4.2 + 14, 50, 7);
            this.drawLine(this.roadlines[i][0], canvas.height / 3.82 + 14, this.roadlines[i][1], canvas.height / 3.82 + 14, 50, 7);

            // vertical filler
            this.drawLine(this.roadlines[i][0] + 3.5, canvas.height / 4.2 + 14, this.roadlines[i][0] + 3.5, canvas.height / 3.82 + 14, 50, 7);
            this.drawLine(this.roadlines[i][1] - 3.5, canvas.height / 4.2 + 14, this.roadlines[i][1] - 3.5, canvas.height / 3.82 + 14, 50, 7);


            // bottom lines
            // horizontal white lines
            this.drawLine(this.roadlines[i][0] + 9, canvas.height / 1.333 - 14, this.roadlines[i][1] - 9, canvas.height / 1.333 - 14, 255, 14);

            // horizontal grey lines
            this.drawLine(this.roadlines[i][0], canvas.height / 1.354 - 14, this.roadlines[i][1], canvas.height / 1.354 - 14, 50, 7);
            this.drawLine(this.roadlines[i][0], canvas.height / 1.313 - 14, this.roadlines[i][1], canvas.height / 1.313 - 14, 50, 7);

            // vertical filler
            this.drawLine(this.roadlines[i][0] + 3.5, canvas.height / 1.354 - 14, this.roadlines[i][0] + 3.5, canvas.height / 1.313 - 14, 50, 7);
            this.drawLine(this.roadlines[i][1] - 3.5, canvas.height / 1.354 - 14, this.roadlines[i][1] - 3.5, canvas.height / 1.313 - 14, 50, 7);
	...
	generateCar(){
        ...
        this.cars.push(new MidnightMotoristsCar(x, this.roadlanes[roadlane], dir, speed, this.rainTheme));
    }
