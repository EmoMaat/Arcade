class PacManAI{
	constructor(wallSettings, spaceMap, intersectionMap, tunnel, ghosts){
		...
		this.box = {
			...
			release(){
				if(this.container.length != 0){
					pacManGame.ai.ghosts[this.container[0]].leavebox = true;
		...
		this.attackTable = {
			...
			stage:0,		// stage of the mode we are in
			...
			if (this.fleeing.timer == 600){ // if we did not touch the timer yet
				...
				for(var i = 0; i < this.ghosts.length; i++){
					if(!this.ghosts[i].inbox){
						this.ghosts[i].fleeing = this.fleeing.state;
...
class PacManAI_Oikake{
	constructor(wallSettings, spaceMap, intersectionMap, tunnel, size = canvas.width / 128 /* = 15*/){
		...
		// stages of Oikake going nuts
		// 1 -> when in scattering will still chase the player and speed increases
		// 2 -> is faster than pacman, inherits 1
		this.Cruise_Elroy = 0; 
		...
		this.home = [{x: 26, y: 2}, {x: 27, y: 2}];	// place to go if scatteringing
	...
	updatePath(pacman){
		...
		// if our target is in the box we cannot enter it
		if(!(this.box[0].x < pacman.x && this.box[1].x > pacman.x && 
			this.box[0].y < pacman.x && this.box[1].y > pacman.y) && this.atIntersection()){
			// if pacman leaves the target tile, calculate a new path to that target tile
			if(pacman.x != this.target.x || pacman.y != this.target.y){
		...
		if(this.debug){
			for(var i = 0; i < this.path.length; i++){
				...
				ctx.font = "30px Arial";
				ctx.beginPath();
				ctx.fillText(this.path[i].back, this.path[i].x * 28 + 9, (this.path[i].y + 0.5) * 28 + 9);
				ctx.stroke();
			}
			...
			for(var i = 0; i < this.PacMan_PathFinding.closedset.length; i++){
				...
				ctx.font = "10px Arial";
					ctx.beginPath();
					ctx.fillText(this.PacMan_PathFinding.closedset[i].back,this.PacMan_PathFinding.closedset[i].x * 28 + 9, (this.PacMan_PathFinding.closedset[i].y + 0.5) * 28 + 9);
					ctx.stroke();
			}
		}
	}
	...
	stuckInTunnel(){
		...
		if(this.inTunnel() && !this.fleeing){
			...
			if(this.back == 1 && this.x <= 6 && this.node.y == 15){
				this.x -= this.speed;
				this.path = [];
				this.target_node.x = this.node.x - 1;
			...
			if(this.back == 3 & this.x >= 23 && this.node.y == 15){
				this.x += this.speed;
				this.path = [];
				this.target_node.x = this.node.x + 1;
...
class PacManAI_Machibuse{
	...
	updatePath(pacman){
		...
		if(this.scattering && this.atIntersection()){
			...
			if(this.pacmanTarget.x != this.target.x || this.pacmanTarget.y != this.target.y){
				if(!this.stuckInTunnel() && !this.fleeing && !this.scattering && !this.eaten.state){
					// debugging will return the whole class instead of only a path
					if(this.debug){
