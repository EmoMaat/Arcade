class MenuInterface {
	constructor() {
		if(arcade.hub.active || arcade.menu.active || arcade.highscores.active || arcade.howtoplay.active || arcade.esc.active)
			return console.log("Trying to instanciate the MenuInterface while another arcade is still active.")

		move.multiplayer = false;
		move.continuous = false;
		arcade.menu.active = true;
	...
	exit(){
		...
		if(typeof arcade.game.object.exit !== "undefined")
			if(typeof arcade.game.object.exit === "function")
				arcade.menu.object.backgroundGame.exit();

		arcade.menu.active = false;
		arcade.menu.object = {};
	...
	set backgroundGame(object){
		...
		arcade.game.interval = this.interval;
		move.continuous = false;
		move.multiplayer = false;

		// Reset the overridden eventlisteners which were possibly overridden by the now added game
...
class EscInterface {
	constructor() {
		if(arcade.hub.active || arcade.menu.active || arcade.highscores.active || arcade.howtoplay.active || arcade.esc.active)
			return console.log("Trying to instanciate the EscInterface while another arcade is still active.")
		...
		// buttons present in the arcade
		this.buttons = [
			new Button(this.EscOptionsCanvas.width / 2 - this.EscOptionsCanvas.width / 9.8, this.EscOptionsCanvas.height / 2.16, this.EscOptionsCanvas.width / 4.8, this.EscOptionsCanvas.height / 36, "Continue", "arcade.esc.object.exit()", self.EscOptionsCtx),
			new Button(this.EscOptionsCanvas.width / 2 - this.EscOptionsCanvas.width / 9.8, this.EscOptionsCanvas.height / 2.16 + this.EscOptionsCanvas.height / 13.5, this.EscOptionsCanvas.width / 4.8, this.EscOptionsCanvas.height / 36, "Exit to menu", "arcade.esc.object.menu()", self.EscOptionsCtx),
			new Button(this.EscOptionsCanvas.width / 2 - this.EscOptionsCanvas.width / 9.8, this.EscOptionsCanvas.height / 2.16 + (this.EscOptionsCanvas.height / 13.5) * 2, this.EscOptionsCanvas.width / 4.8, this.EscOptionsCanvas.height / 36, "Exit to hub", "arcade.esc.object.hub()", self.EscOptionsCtx)
		];
		...
		arcade.game.interval.pause()
		arcade.esc.active = true;
		arcade.esc.object = this;
	}
	update() {
		// draw the buttons
	...
	exit() {
		...
		arcade.game.interval.resume() //we resume the game
	}
	menu() {
		exit_open_game();
		exit_open_arcade();
...
class HighScoresInterface{
	constructor(game, scoredPoints){
		...
		this.canvas_Header();
		arcade.highscores.active = true;
		arcade.highscores.object = this;
	}
	...
	canvas_Header(){
		...
		this.HighScoreHeaderCtx.font = "70px SegoeUI"
	...
	exit(){
		...
		move._up.setEventListener(null);
		move._down.setEventListener(null);
		arcade.highscores.active = false;
		arcade.highscores.object = {};
	}
...
class HighScoreWindow{
	constructor(score, names, scores){
		...
		this.HighScoreListCtx.font = "50px SegoeUI";
		...
		this.buttons = [
			new Button(this.canvas.width/2 - this.canvas.width / 4,200,130,40,"     HUB","arcade.highscores.object.toHub()", this.ctx),
			new Button(this.canvas.width/2 - 65,200,130,40,"  RESTART","arcade.highscores.object.restartGame()", this.ctx),
			new Button(this.canvas.width/2 + this.canvas.width / 4 - 130,200,130,40,"    MENU","arcade.highscores.object.toMenu()", this.ctx)
		];
...
class NameWindow{
	constructor(score){
		...
		this.optionButtons.push(
			new Button(this.canvas.width/2 - 230 - 120,450,230,40,"SAVE","arcade.highscores.object.saveHighScore()", this.ctx),
			new Button(this.canvas.width/2 + 120,450,270,40,"DON'T SAVE","arcade.highscores.object.canvas_loadHighScores()", this.ctx),
		);
	...
	async createCanvases(){
		...
		this.HighScoreSaverCtx.font = "50px SegoeUI";
...
class NameWindowButton{
	...
	update(){
		...
		if(this.selected)
			this.ctx.font = "bold 90px SegoeUI";
		else
			this.ctx.font = "90px SegoeUI";
...
class HubButton {
	constructor(x, y, text, game, ctx){
		...
		if(this.ctx.measureText(text).width <= this.width - this.width * 0.1)
	...
	renderText(){
		// Calculate the offset and spacing
		let textHeight = 50;
		let offset = 0;

		this.ctx.fillStyle = "#ccc";
		this.ctx.font = "30px SegoeUI";

		if(this.selected){
			offset = this.y + this.height / 2 + textHeight / 2;
		} else {
			offset = this.y + this.height + 45;
		}

		for(let t = 0, l = this.text.length; t < l; t++){
			this.ctx.fillText(this.text[t].toUpperCase(), this.x + this.width / 2, offset + (textHeight / 2) * t)
		}
	}
}
