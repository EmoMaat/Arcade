...
class EscInterface {
	constructor() {
		...
		// interfaces.esc.active = true; 	// set this to true so we won't create a second instance if we hit esc again 
		...
		// .exit() is mostly used for stopping audio and removing canvasses as clearing classes won't work
		// if(typeof eval(currentGame.charAt(0).toLowerCase() + currentGame.slice(1) + "Game.exit") === "function")
			// eval(currentGame.charAt(0).toLowerCase() + currentGame.slice(1) + "Game.exit()");
		...
		this.buttons = [
			new Button(this.EscOptionsCanvas.width / 2 - this.EscOptionsCanvas.width / 9.8, this.EscOptionsCanvas.height / 2.16, this.EscOptionsCanvas.width / 4.8, this.EscOptionsCanvas.height / 36, "Continue", "interfaces.esc.object.close()", self.EscOptionsCtx),
...
class HighScoresInterface{
	constructor(game, scoredPoints){
		if(interfaces.hub.active || interfaces.menu.active || interfaces.highscores.active || interfaces.howtoplay.active || interfaces.esc.active)
			return console.log("Trying to instanciate the HighScoresInterface while another interfaces is still active.")
...
class HighScoreWindow{
	...
	createCanvases(){
		...
		this.HighScoreOptions.style.top = Math.floor(canvas.height / 5.4 + canvas.height / 2.4);
...
class NameWindow{
	...
	async createCanvases(){
		...
		this.HighScoreOptions.style.paddingTop = Math.floor(canvas.height / 21.6);
...
class HubButton {
	...
	update() {
		...
		if (this.selected) {
			...
			this.ctx.fillText(this.text,this.x + this.width/2,this.y + this.height + (this.width / this.height * 70));
			this.ctx.strokeRect(
			...
			this.ctx.fillText(this.text,this.x + this.width/2,this.y + this.height + 40);
			this.ctx.strokeRect(
				this.x,
				this.y,
