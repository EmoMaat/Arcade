...
// ----- BEGIN ARCADE SCREEN SIZE -----

window.width = window.innerWidth;
window.height = window.innerHeight;
window.scale = window.height / 1080;

// ------ END ARCADE SCREEN SIZE ------

// Load the default font
new FontFace('SegoeUI', 'url(segoeui.ttf)');
...
function exit_open_game(){
    if(typeof arcade.game.object.exit !== "undefined")
        if(typeof arcade.game.object.exit === "function")
			arcade.game.object.exit();

	if(typeof arcade.menu.object.backgroundGame !== "undefined")
		if(typeof arcade.menu.object.backgroundGame.exit !== "undefined")
			if(typeof arcade.menu.object.backgroundGame.exit === "function")
				arcade.menu.object.backgroundGame.exit();

    // stop any intervals running
    if(arcade.game.interval != null)
        if(arcade.game.interval.state !== 0)
            arcade.game.interval.stop();
}

function exit_open_arcade(){
	for(let key in arcade){
		if(arcade[key].active == true){
			arcade[key].object.exit();
			arcade[key].object = {}
			arcade[key].active = false;
		}
	}
}
