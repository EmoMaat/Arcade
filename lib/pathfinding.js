class PathFinding{
	 ...
	 */
	constructor(start, end, map, heuristicsType = "manhattan", debug = false){
	...
	findPath() {
		...
		if(this.end.x == this.start.x && this.end.y == this.start.y){
			console.log("Goal is same as start."); 
			return [];
		}
		...
		if(is_reachable){
			...
			console.log("Goal not reachable, unable to create path: x => " 
	...
	heuristics(node0, node1, heuristics){
		...
		// can move with 8 angles
		if(heuristics == "diagonal"){
			dx = abs(node0.x - node1.x)
			dy = abs(node0.y - node1.y)
			return D * (dx + dy) + (D2 - 2 * D) * min(dx, dy)
		}

		// can move with any angle
	...
	getNeighbours(node) {
		...
		// cost should have as identifier
		// "cost". undefined will result in cost = 1;
		for (let i = 0; i < this.map.length; i++){
			// left
			if (this.map[i].x == node.x - 1 && this.map[i].y == node.y) {
				this.retPush(this.map[i].x, this.map[i].y, this.map[i].cost);
			}	
			...
			// down
			if (this.map[i].x == node.x && this.map[i].y == node.y + 1) {
				this.retPush(this.map[i].x, this.map[i].y, this.map[i].cost);
			}	
			...
			if (this.diagonal) {
				// top left
				if (this.map[i].x == node.x - 1 && this.map[i].y == node.y - 1) {
					this.retPush(this.map[i].x, this.map[i].y, this.map[i].cost);
				}
	...
	retPush(addx, addy, addcost){
		if (typeof this.map[i].cost == undefined)
			this.ret.push({x: addx, y:addy, cost:1});
		else 
			this.ret.push({x: addx, y:addy, cost:addcost});
	}
}
