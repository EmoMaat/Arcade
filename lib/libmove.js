...
class MoveKey{
	constructor(){
		...
		this._continuous = false;	// whether continuous keylogging is enabled for this particular key
		this._cooldown = {
			state: false,
			defTimer: 40,
			timer: 40
	...
	set state(state){
		...
		if(this._cooldown.state && state){
			if(this._cooldown.timer == this._cooldown.defTimer){
				this._state = true;
				this._cooldown.timer--;
			} else if(this._cooldown.timer >= 0){
				this._state = false;
				this._cooldown.timer--;
			} else if (this._cooldown.timer < 0){
				this._state = true;
		...
		if (this._cooldown.state && !state){
			this._cooldown.timer = this._cooldown.defTimer;
			this._state = false;
		}
		...
		if(this.listener !== null && this.state && state){
			this.listener();
		}
	}
...
class Move{
	 ...
	 */
	set continuous(state){
		this.player[0].continuous = state;
		this.player[0]._left.continuous = state;
		this.player[0]._right.continuous = state;
		this.player[0]._up.continuous = state;
		this.player[0]._down.continuous = state;
	}
	get continuous (){ return this.player[0]._continuous}
